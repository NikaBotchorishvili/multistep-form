...
function AddOnsSummary() {
	const { formData, goto, typeAbbreviation} = useContext(Context) as Required<Partial<ContextType>>;
		...
		<article className="flex flex-col gap-2">
			{content.length == 0 ? (
				<div className="flex flex-col items-start gap-y-1">
					<small className="text-lg text-MarineBlue font-medium">No add-ons selected</small>
					<button onClick={() => goto(2)} className="underline text-CoolGray hover:text-PurplishBlue">Select</button>
				</div>
			) : (
				content
			)}
